...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def minimax(self, gameState, agentIndex, depth):
        if depth is self.depth * gameState.getNumAgents()\
                or gameState.isLose() or gameState.isWin():#check for termination reasons
            return self.evaluationFunction(gameState)
            
        if agentIndex is 0: # if pacman (max agent) - return max successor value
            return self.maxval(gameState, agentIndex, depth)[1]
        else:# if ghost (EXP agent) - return min successor value
            return self.minval(gameState, agentIndex, depth)[1]

    #get the maximum-value action to choose from all legal actions
    def maxval(self, gameState, agentIndex, depth):
        bestmv = ("max",-float("inf"))
        
        for move in gameState.getLegalActions(agentIndex):
            succAction = (move,self.minimax(gameState.generateSuccessor(agentIndex,move),
                (depth + 1)%gameState.getNumAgents(),depth+1))
            bestmv = max(bestmv,succAction,key=lambda x:x[1])
        return bestmv
    
    #get the minimum-value action to choose from all legal actions
    def minval(self, gameState, agentIndex, depth):
        bestmv = ("min",float("inf"))
        for move in gameState.getLegalActions(agentIndex):
            succAction = (move,self.minimax(gameState.generateSuccessor(agentIndex,move),
                (depth + 1)%gameState.getNumAgents(),depth+1))
            bestmv = min(bestmv,succAction,key=lambda x:x[1])
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def maxval(self, gameState, agentIndex, depth, alpha, beta):
        ...
        for move in gameState.getLegalActions(agentIndex):
            succAction = (move,self.alphabeta(gameState.generateSuccessor(agentIndex,move),
                                      (depth + 1)%gameState.getNumAgents(),depth+1, alpha, beta))
            bestmv = max(bestmv,succAction,key=lambda x:x[1])

            
            # Prunning
            if bestmv[1] > beta: return bestmv
            else: alpha = max(alpha,bestmv[1])
    ...
    def minval(self, gameState, agentIndex, depth, alpha, beta):
        ...
        for move in gameState.getLegalActions(agentIndex):
            ...
            # Prunning
            if bestmv[1] < alpha: return bestmv
            else: beta = min(beta, bestmv[1])
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def expectimax(self, gameState, move, depth, agentIndex):
        ...
        if agentIndex is 0:
            return self.maxvalue(gameState,move,depth,agentIndex)
        # if ghost (EXP agent) - return probability value
        else:
            return self.expvalue(gameState,move,depth,agentIndex)

    def maxvalue(self,gameState,move,depth,agentIndex):
        bestmv = ("max", -(float('inf')))
        for legalAction in gameState.getLegalActions(agentIndex):
            ...
            bestmv = max(bestmv,succValue,key = lambda x:x[1])
        return bestmv

    def expvalue(self,gameState,move,depth,agentIndex):
        legalActions = gameState.getLegalActions(agentIndex)
        averageScore = 0
        propability = 1.0/len(legalActions)
        for legalAction in legalActions:
            nextAgent = (agentIndex + 1) % gameState.getNumAgents()
            bestmv = self.expectimax(gameState.generateSuccessor(agentIndex, legalAction),
                                         move, depth - 1, nextAgent)
            averageScore += bestmv[1] * propability
        return (move, averageScore)
...
def betterEvaluationFunction(currentGameState):
    ...
    # significant information can be extracted from the GameState (pacman.py)
